 ...
 */
    long o_l_threshold = 0, count = 0;
    for (blob = word->chopped_word->blobs, index = 0;
        blob != NULL; blob = blob->next, index++) {
        for (TESSLINE* o_l = blob->outlines; o_l != NULL; o_l = o_l->next)
        {
            if (!(o_l->is_hole))
            {
                o_l_threshold += o_l->BBArea();
                count++;
            }
        }
    }
    o_l_threshold = (o_l_threshold / count) >> 2;
    printf("%d\n", o_l_threshold);
    ...
        int cnt = 0;
    for (TESSLINE* o_l = blob->outlines, *o_tmp; o_l != NULL; o_l = o_l->next, cnt++)
    {
        if (o_l->BBArea() < o_l_threshold && !(o_l->is_hole))
        {
            if (cnt == 0) {
                blob->outlines = o_l->next;
                cnt--;
            }
            else
                o_tmp->next = o_l->next;
    ...
    {
        if (index == 0)
            word->chopped_word->blobs = blob->next;
        else
            blob_tmp->next = blob->next;
        //blob->Clear();
        index--;
        continue
    }
